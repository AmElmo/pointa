...
This document describes the comprehensive update notification system implemented in Pointa to keep users informed about extension and server updates.
...
async checkForUpdates() {
  ...
  const response = await fetch(
    'https://registry.npmjs.org/pointa-server/latest'
  );
...
║  Run: npm update -g pointa-server                           ║
╚════════════════════════════════════════════════════════════════╝
    `));
  }
}
...
- **24-Hour Cache** - Prevents spamming NPM registry (stored in `~/.pointa/.update-check`)
...
```
GET https://registry.npmjs.org/pointa-server/latest
```
...
- Cache file: `~/.pointa/.update-check`
- NPM Registry endpoint: `https://registry.npmjs.org/pointa-server/latest`
...
ls -la ~/.pointa/.update-check

# Check NPM Registry manually  
curl https://registry.npmjs.org/pointa-server/latest
```
